...
var _sha = require("../hash/sha256/sha256");

class HmacSha256 extends _hmac.Hmac {
  constructor(password, verify) {
    const hash = new _sha.Sha256(); // Calculate ipad, init the underlying engine, calculate this.key
  ...
  finish() {
    if (this.key === null) throw new _errors.IllegalStateError('no key is associated with the instance');
    ...
    const result = new Uint8Array(_sha._sha256_hash_size);
    result.set(heap.subarray(0, _sha._sha256_hash_size));
