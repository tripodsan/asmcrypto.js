...
exports.RSA_PKCS1_v1_5 = exports.RSA_PSS = exports.RSA_OAEP = void 0;
...
var _getRandomValues = require("../other/get-random-values");

class RSA_OAEP {
  constructor(key, hash, label) {
    this.rsa = new _rsa.RSA(key);
    this.hash = hash;
  ...
  encrypt(data, random) {
    ...
    if (data_length > key_size - 2 * this.hash.HASH_SIZE - 2) throw new _errors.IllegalArgumentError('data too large');
    ...
    if (random !== undefined) {
      if (seed.length !== random.length) throw new _errors.IllegalArgumentError('random size must equal the hash size');
      seed.set(random);
    } else {
      (0, _getRandomValues.getRandomValues)(seed);
    }
    ...
    this.rsa.encrypt(new _bignum.BigNumber(message));
    return new Uint8Array(this.rsa.result);
  }

  decrypt(data) {
    if (!this.rsa.key) throw new _errors.IllegalStateError('no key is associated with the instance');
    const key_size = Math.ceil(this.rsa.key[0].bitLength / 8);
    const hash_size = this.hash.HASH_SIZE;
    const data_length = data.byteLength || data.length || 0;
    if (data_length !== key_size) throw new _errors.IllegalArgumentError('bad data');
    this.rsa.decrypt(new _bignum.BigNumber(data));
    ...
    if (z !== 0) throw new _errors.SecurityError('decryption failed');
    const seed_mask = this.RSA_MGF1_generate(data_block, seed.length);
    ...
    for (let i = 0; i < hash_size; i++) {
      if (lhash[i] !== data_block[i]) throw new _errors.SecurityError('decryption failed');
    }
    ...
    for (; ps_end < data_block.length; ps_end++) {
      ...
      if (psz === 1) break;
      if (psz !== 0) throw new _errors.SecurityError('decryption failed');
    }

    if (ps_end === data_block.length) throw new _errors.SecurityError('decryption failed');
    this.rsa.result = data_block.subarray(ps_end + 1);
    return new Uint8Array(this.rsa.result);
  }

  RSA_MGF1_generate(seed, length = 0) {
    const hash_size = this.hash.HASH_SIZE; //    if ( length > (hash_size * 0x100000000) )
...
class RSA_PSS {
  ...
  sign(data, random) {
    ...
    if (salt_length > 0) {
      if (random !== undefined) {
        if (m_salt.length !== random.length) throw new _errors.IllegalArgumentError('random size must equal the salt size');
        m_salt.set(random);
      } else {
        (0, _getRandomValues.getRandomValues)(m_salt);
      }
    }
    ...
    this.rsa.decrypt(new _bignum.BigNumber(message));
  ...
  verify(signature, data) {
    ...
    const salt_length = this.saltLength;
    const ps_length = message_length - salt_length - hash_size - 2;
    this.rsa.encrypt(new _bignum.BigNumber(signature));
    const message = this.rsa.result;
    if (message[message_length - 1] !== 0xbc) throw new _errors.SecurityError('bad signature');
    const h_block = message.subarray(message_length - hash_size - 1, message_length - 1);
    const d_block = message.subarray(0, message_length - hash_size - 1);
    const d_salt = d_block.subarray(ps_length + 1);
    const zbits = 8 * message_length - key_bits + 1;
    if (zbits % 8 && message[0] >>> 8 - zbits) throw new _errors.SecurityError('bad signature');
    const d_block_mask = this.RSA_MGF1_generate(h_block, d_block.length);
    ...
    for (let i = 0; i < ps_length; i++) {
      if (d_block[i] !== 0) throw new _errors.SecurityError('bad signature');
    }

    if (d_block[ps_length] !== 1) throw new _errors.SecurityError('bad signature');
    const m_block = new Uint8Array(8 + hash_size + salt_length);
    const m_hash = m_block.subarray(8, 8 + hash_size);
    const m_salt = m_block.subarray(8 + hash_size);
    ...
    for (let i = 0; i < hash_size; i++) {
      if (h_block[i] !== h_block_verify[i]) throw new _errors.SecurityError('bad signature');
    }
  }
...
class RSA_PKCS1_v1_5 {
  ...
  sign(data) {
    if (!this.rsa.key) {
      throw new _errors.IllegalStateError('no key is associated with the instance');
    }
    ...
    this.rsa.decrypt(new _bignum.BigNumber(em));
    return this.rsa.result;
  }
  ...
  verify(signature, data) {
    ...
    if (k < t_len + 11) {
      throw new _errors.SecurityError('Bad signature');
    }

    this.rsa.encrypt(new _bignum.BigNumber(signature));
    const m_hash = new Uint8Array(hash_size);
    m_hash.set(this.hash.process(data).finish().result);
    let res = 1; // EM = 0x00 || 0x01 || PS || 0x00 || T
