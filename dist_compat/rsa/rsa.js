...
class RSA {
  constructor(key) {
    ...
    const k0 = new _bignum.Modulus(new _bignum.BigNumber(key[0]));
    const k1 = new _bignum.BigNumber(key[1]);
    this.key = {
      0: k0,
      1: k1
    };
    ...
    if (l > 2) {
      this.key[2] = new _bignum.BigNumber(key[2]);
    }

    if (l > 3) {
      this.key[3] = new _bignum.Modulus(new _bignum.BigNumber(key[3]));
      this.key[4] = new _bignum.Modulus(new _bignum.BigNumber(key[4]));
      this.key[5] = new _bignum.BigNumber(key[5]);
      this.key[6] = new _bignum.BigNumber(key[6]);
      this.key[7] = new _bignum.BigNumber(key[7]);
    }
  }

  encrypt(msg) {
    if (!this.key) throw new _errors.IllegalStateError('no key is associated with the instance');
