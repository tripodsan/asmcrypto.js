...
exports.AES_CCM = void 0;

var _aes = require("./aes.asm");

var _aes2 = require("./aes");
...
class AES_CCM extends _aes2.AES {
  constructor(key, nonce, adata, tagSize = 16, dataLength) {
    super(key, undefined, undefined, 'CCM');
    this.counter = 1;
    this.dataLength = -1; // Tag size

    if (tagSize < 4 || tagSize > 16 || tagSize & 1) throw new _errors.IllegalArgumentError('illegal tagSize value');
    this.tagSize = tagSize; // Nonce

    this.nonce = nonce;
    if (nonce.length < 8 || nonce.length > 13) throw new _errors.IllegalArgumentError('illegal nonce length');
    this.lengthSize = 15 - nonce.length;
    nonce = new Uint8Array(nonce.length + 1);
    nonce[0] = this.lengthSize - 1;
    nonce.set(this.nonce, 1);
    if (dataLength < 0 || dataLength > _AES_CCM_data_maxLength || dataLength > Math.pow(2, 8 * this.lengthSize) - 16) throw new _errors.IllegalArgumentError('illegal dataLength value');

    if (adata !== undefined) {
      if (adata.length > _AES_CCM_adata_maxLength) throw new _errors.IllegalArgumentError('illegal adata length');
  ...
  AES_CCM_calculate_iv() {
    ...
    this.asm.get_state(_aes.AES_asm.HEAP_DATA);
  ...
  _cbc_mac_process(data) {
    ...
    while (dlen > 0) {
      ...
      dpos += wlen;
      dlen -= wlen;
      asm.mac(_aes.AES_asm.MAC.CBC, _aes.AES_asm.HEAP_DATA, wlen);
    }
  }
  ...
  AES_CCM_Encrypt_process(data) {
    ...
    while (dlen > 0) {
      ...
      dpos += wlen;
      dlen -= wlen;
      wlen = asm.mac(_aes.AES_asm.MAC.CBC, _aes.AES_asm.HEAP_DATA + pos, len);
      wlen = asm.cipher(_aes.AES_asm.ENC.CTR, _aes.AES_asm.HEAP_DATA + pos, wlen);
  ...
  AES_CCM_Encrypt_finish() {
    ...
    asm.mac(_aes.AES_asm.MAC.CBC, _aes.AES_asm.HEAP_DATA + pos, i);
    asm.cipher(_aes.AES_asm.ENC.CTR, _aes.AES_asm.HEAP_DATA + pos, i);
    if (len) result.set(heap.subarray(pos, pos + len));
    asm.set_counter(0, 0, 0, 0);
    asm.get_iv(_aes.AES_asm.HEAP_DATA);
    asm.cipher(_aes.AES_asm.ENC.CTR, _aes.AES_asm.HEAP_DATA, 16);
  ...
  AES_CCM_Decrypt_process(data) {
    ...
    while (dlen > tlen) {
      ...
      dpos += wlen;
      dlen -= wlen;
      wlen = asm.cipher(_aes.AES_asm.DEC.CTR, _aes.AES_asm.HEAP_DATA + pos, wlen);
      wlen = asm.mac(_aes.AES_asm.MAC.CBC, _aes.AES_asm.HEAP_DATA + pos, wlen);
      if (wlen) result.set(heap.subarray(pos, pos + wlen), rpos);
      counter += wlen >>> 4;
      rpos += wlen;
      pos = 0;
  ...
  AES_CCM_Decrypt_finish() {
    ...
    asm.cipher(_aes.AES_asm.DEC.CTR, _aes.AES_asm.HEAP_DATA + pos, rlen + 15 & -16);
